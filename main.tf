...
resource "aws_subnet" "private_data_1" {
  ...
  tags = {
    Name = "${var.project_name}-private-data-1"
  }
}
...
resource "aws_subnet" "private_data_2" {
  ...
  tags = {
    Name = "${var.project_name}-private-data-2"
  }
}
...
resource "aws_security_group" "app_sg" {
  ...
  description = "Allow traffic from the ALB to the app tier"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "data_sg" {
  ...
  description = "Allow app tier access to the data tier"
  vpc_id      = aws_vpc.main.id
  ...
  container_definitions = jsonencode([
    {
      ...
      command = [
        "/bin/sh",
        "-c",
        "echo '<h1>Secure SaaS Platform Running</h1>' > /usr/share/nginx/html/index.html && nginx -g 'daemon off;'"
      ]
...
resource "aws_wafv2_web_acl" "main" {
  ...
  description = "WAF for SaaS platform"
  scope       = "REGIONAL"
...
resource "aws_s3_bucket" "security_logs" {
  bucket = "${var.project_name}-security-logs-${random_id.log_bucket_suffix.hex}"
